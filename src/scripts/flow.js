import { addMonthsUTC, hideElement, on, qs, requireDateInput, showElement, subtractYearsUTC, toUTC } from './utils.js';
...
export class PNFFlow {
    ...
    constructor({
        ...
        for (const [key, id] of Object.entries(inputIds)) {
            this.inputs[key] = /** @type {HTMLInputElement | null} */ (qs(`#${id}`));
        }
     ...
     */
    bindHandlers() {
        on(qs('#question1'), 'click', '[data-answer]', (e) => this.handleQ1(e.target.dataset.answer));
        on(qs('#question2'), 'click', 'button[data-action="next"]', () => this.handleQ2());
        on(qs('#question3'), 'click', 'button[data-action="next"]', () => this.handleQ3());
        on(qs('#question4'), 'click', '[data-submission]', (e) => this.handleQ4(e.target.dataset.submission));
        on(qs('#question5'), 'click', '[data-everclaimed]', (e) => this.handleQ5(e.target.dataset.everclaimed));
    }
    ...
    handleQ3() {
        ...
        const cpStartDateUTC = this.getValidatedUTCDate('cpStart', 'Please enter the claim period start date.');
        const cpEndDateUTC = this.getValidatedUTCDate('cpEnd', 'Please enter the claim period end date.');
    ...
    handleQ5(answer) {
        if (answer === 'yes') {
            alert('Please now enter the details for the claim made *before* the one you just described in the previous steps.');
